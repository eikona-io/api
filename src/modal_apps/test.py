...
def flux_params(_):
    return "beautiful scenery nature glass bottle landscape, purple galaxy bottle"
...
def main():
    ...
    for name, (_time, result) in zip(comfy_runners.keys(), results):
        print_loading_times(f"{name} Loading Times {_time:.2f} seconds", result.get('loading_time', {}))
