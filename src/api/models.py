from sqlalchemy import Column, String, Enum, DateTime, Boolean, MetaData, Float, JSON, ForeignKey, Integer, func
...
class Workflow(Base):
    ...
    selected_machine_id = Column(UUID(as_uuid=True), ForeignKey('machines.id', ondelete='set null'))
...
class WorkflowRun(Base):
    ...
    origin = Column(Enum('manual', 'api', 'public-share', 'public-template', 'workspace', 'workspace-v2', name='workflow_run_origin'), nullable=False, default='api')
    status = Column(Enum('not-started', 'running', 'uploading', 'success', 'failed', 'started', 'queued', 'timeout', 'cancelled', name='workflow_run_status'), nullable=False, default='not-started')
    ...
    machine_type = Column(Enum('classic', 'runpod-serverless', 'modal-serverless', 'comfy-deploy-serverless', 'workspace', 'workspace-v2', name='machine_type'))
    ...
    webhook_status = Column(Enum('success', 'failed', 'not-started', 'running', name='webhook_status'))
...
class Deployment(Base):
    ...
    environment = Column(Enum('staging', 'production', 'public-share', 'private-share', name='deployment_environment'), nullable=False)
    created_at = Column(DateTime, nullable=False)
    updated_at = Column(DateTime, nullable=False)
...
class Machine(Base):
    ...
    type = Column(Enum('classic', 'runpod-serverless', 'modal-serverless', 'comfy-deploy-serverless', 'workspace', 'workspace-v2', name='machine_type'), nullable=False, default='classic')
    status = Column(Enum('not-started', 'ready', 'building', 'error', 'running', 'paused', 'starting', name='machine_status'), nullable=False, default='ready')
    static_assets_status = Column(Enum('not-started', 'ready', 'building', 'error', 'running', 'paused', 'starting', name='machine_status'), nullable=False, default='not-started')
    machine_version = Column(String)
    machine_builder_version = Column(Enum('2', '3', name='machine_builder_version'), default='2')
    snapshot = Column(JSON)
    models = Column(JSON)
    gpu = Column(Enum('T4', 'L4', 'A10G', 'A100', 'H100', name='machine_gpu'))
    ...
    target_workflow_id = Column(UUID(as_uuid=True), ForeignKey('workflows.id', ondelete='set null'))
    dependencies = Column(JSON)
