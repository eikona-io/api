...
from api.models import Model as ModelDB
...
async def getVolumeList(volume_name: str) -> VolFSStructure:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            logger.error(f"HTTP error {e.response.status_code} while fetching volume list for {volume_name}")
            raise HTTPException(status_code=e.response.status_code, detail=str(e))
        except Exception as e:
            logger.error(f"Unexpected error while fetching volume list for {volume_name}: {str(e)}")
